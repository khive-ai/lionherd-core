...
| **Pile.__list__/to_list** | Removed | Use `list(pile)` |
| **Progression.__init__** | Removed custom init | No change needed (field validator handles it) |
| **Progression validation** | Stricter | Ensure valid UUIDs only |
| **Progression.__bool__** | Added | Use `if not prog:` instead of `if len(prog) == 0:` |
| **Progression.pop/popleft** | IndexError → NotFoundError | Update exception handling |
| **Flow.__init__** | New progressions param | `Flow(progressions=[...])` |
...
```python
# Before: pile.to_list(), pile.__list__()
# After:  list(pile)
```
